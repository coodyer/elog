...
public class ConcurrentUtil {
	...
	public static Object invokMethod(Object targeObj, String methodName,
		...
		System.out.println("缓存锁:" + lockKey);
		//获取进入许可证
		if (!checkPermit(lockKey)) {
			System.out.println("等待超时:" + lockKey);
			return null;
		}
		try {
			// 寻找方法
			...
			if (StringUtils.isNullOrEmpty(method)) {
				System.out.println("方法不存在:" + targeObj.getClass().getName()
						+ "." + methodName);
				return null;
			}
	...
	private static boolean isTargeMethod(Method method, String methodName,
		...
		for (int i = 0; i < clas.length; i++) {
			System.out.println("方法参数检测:"+paras[i].getClass().getName()+":"+clas[i].getName());
