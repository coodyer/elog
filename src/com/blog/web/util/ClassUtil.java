...
public class ClassUtil {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 111; i < 999; i++) {
			if (i % 111 == 0 || String.valueOf(i).indexOf("0")>-1) {
				continue;
			}
			for (int n = 111; n < 1000-i; n++) {
				if (i % 111 == 0 || String.valueOf(n).indexOf("0")>-1||String.valueOf(n + i).indexOf("0")>-1) {
					continue;
				}
				str = MessageFormat.format("{0}+{1}={2}", i, n, n + i);
				if(checkStr(str)){
					System.out.println(str);
					str = MessageFormat.format("{0}+{1}={2}", n, i, n + i);
					System.out.println(str);
				}
			}
		}
