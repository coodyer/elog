		...
		<div class="am-panel-hd am-cf"
			...
			<a href="${basePath }user/index.${defSuffix}">我的项目</a><span class="am-icon-chevron-down am-fr"></span> <a
				...
				<button type="submit" name=""
					...
					<i class="am-icon-spinner am-icon-spin"></i> 创建
				</button>
			</a>
		</div>
		<div class="am-panel-bd am-collapse am-in" id="collapse-panel-1">
			<ul class="am-list admin-content-file" style="text-align: left">
				<c:if test="${empty projects }">
					<li><small>暂无数据</small></li>
				</c:if>
				<c:forEach items="${projects }" var="project">
					<li><a href="${basePath }user/projectEdit.${defSuffix}?id=${project.id}"><small>${project.title }</small></a></li>
				</c:forEach>
			</ul>
		</div>
	</div>
