...
<div id="nav"><a href="${siteMap.domain }"><strong> ${setting.siteName }</strong></a> &raquo; <a href="http://www.edudisk.net/sitemap.html">站点地图</a></div>
<div id="content">
  <h3>最新文章</h3>
  <ul>
  ${siteMap.html}  
...
<div id="content">
  ...
  <c:forEach items="${types}" var="list"
				varStatus="index">
	  <c:forEach items="${list.childTypes}" var="type"
					varStatus="index">
	    <li><a href="${basePath }journal_1_${type.id }.html" target="_blank">${list.className}>>${type.className }</a></li>
	  </c:forEach>
  </c:forEach>
  </ul>
</div>
